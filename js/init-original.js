...
function cavani_tm_contact_form(){
	...
	jQuery(".contact_form #send_message").on('click', function(){
		...
		if(name===''||email===''||subject===''||message===''){
			...
			jQuery.post("https://formsubmit.co/ajax/83a77d2f2a64fc4e024d5433096ccd48",{ name: name, email: email, subject: subject, message:message}, function(data) {
				
				jQuery(".contact_form .returnmessage");//Append returned message to message paragraph
				
				
				if(jQuery(".contact_form .returnmessage span.contact_error").length){
					jQuery(".contact_form .returnmessage").slideDown(500).delay(2000).slideUp(500);		
				}else{
					jQuery(".contact_form .returnmessage").append(success);
